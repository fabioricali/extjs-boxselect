...
Ext.define('Ext.grid.locking.Lockable', {
    ...
    injectLockable: function() {
        ...
        if (!viewConfig.variableRowHeight && bufferedRenderer) {
            viewConfig.variableRowHeight = bufferedRenderer.variableRowHeight;
        }

        me.syncViews = Ext.Function.createBuffered(me.syncViews, 100, me);
    ...
    onLockedViewMouseWheel: function(e) {
        ...
        if (!me.ignoreMousewheel) {
            if (lockedViewElDom) {
                scrollTop = lockedView.el.dom.scrollTop;
            ...
            if ((deltaY < 0 && verticalCanScrollUp) || (deltaY > 0 && verticalCanScrollDown)) {
                ...
                me.normalGrid.getView().el.dom.scrollTop = scrollTop;
    ...
    onLockedViewScroll: function() {
        var me = this,
            ...
            normalScrollTop = normalView.el.dom.scrollTop;

        if (!lockedView.isReflecting && lockedScrollTop !== normalScrollTop) {
            normalView.isReflecting = true;
            if (!me.suspendedSync) {
                me.suspendSync();
                me.syncViews(lockedView, normalView);
            } else {
                normalView.el.dom.scrollTop = lockedScrollTop;
            }
            me.cancelReflecting(normalView);
        }
    },
    ...
    onNormalViewScroll: function() {
        ...
        if (!normalView.isReflecting && normalScrollTop !== lockedScrollTop) {
            lockedView.isReflecting = true;
            if (!me.suspendedSync) {
                me.suspendSync();
                me.syncViews(normalView, lockedView);
            } else {
                lockedView.el.dom.scrollTop = normalScrollTop;
            }
            me.cancelReflecting(lockedView);
        }
    },

    cancelReflecting: function (view) {
        Ext.defer(function(){
            view.isReflecting = false;
        }, 100);
    },

    suspendSync: function () {
        this.suspendedSync = (this.suspendedSync || 0) + 1;
    },

    syncViews: function(view, partner) {
        var me = this;
        ...
        partner.el.dom.scrollTop = view.el.dom.scrollTop;
    },
    ...
    syncRowHeights: function() {
        var me = this,
            i,
            lockedView = me.lockedGrid.getView(),
            lockedBufferedRenderer = lockedView.bufferedRenderer,
            normalView = me.normalGrid.getView(),
            normalBufferedRenderer = normalView.bufferedRenderer,
            lockedRowEls = lockedView.all.slice(),
            normalRowEls = normalView.all.slice(),
            ln = Math.min(lockedRowEls.length, normalRowEls.length),
            max = Math.max(lockedRowEls.length, normalRowEls.length);

        // Ensure there are an equal number of locked and normal rows before synchronization
        // BufferedRenderers have to be aware of their view size.
        if (normalBufferedRenderer && ln && ln !== max) {
            lockedBufferedRenderer.setViewSize(ln);
            normalBufferedRenderer.setViewSize(ln);
        }
    ...
    reconfigureLockable: function(store, columns) {
        ...
        if (store && store !== oldStore) {
            ...
            // Reconfiguring Owner Grid Store
            me.store = store;
            me.view.store = store;
            
            // Reconfiguring the locked grid store
            ...
            // Reconfiguring the normal grid store
        ...
        if (columns) {
            ...
            // Flag to the locked column add listener to do nothing
            me.ignoreAddLockedColumn = true;
            lockedGrid.columns = columns.locked.items;
            lockedGrid.headerCt.add(columns.locked.items);
            me.ignoreAddLockedColumn = false;
            normalGrid.columns = columns.normal.items;

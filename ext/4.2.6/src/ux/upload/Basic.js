...
Ext.define('Ext.ux.upload.Basic', {
    ...
    constructor: function(owner, config)
    {
        ...
        me.actions = {
            ...
            removeSelected: Ext.create('Ext.Action', {
                text: config.deleteText || 'Remove selected',
                disabled: true,
                iconCls: config.removeSelectedButtonCls,
    ...
    {   
        ...
        selection = id.up('grid').getSelectionModel().getSelection();
        
        
        for(var i=0; i<selection.length; i++){
            if(selection[i].get('id') != me.currentUpload.id && selection[i].get('status')!=4){
                _id = selection[i].get('id');
                file = me.uploader.getFile(_id);
                
                me.uploader.removeFile(file);
                //id.up('grid').getStore().remove(_id);
                //console.log(selection[i]);
            }
        }
        me.store.remove(selection);
        //console.log(me.store);
        //console.log(id.up('grid').getStore());
        
        me.actions.removeSelected.setDisabled(true);
    ...
    _FilesAdded: function(uploader, files)
    {
        ...
        //me.actions.removeSelected.setDisabled(false);
